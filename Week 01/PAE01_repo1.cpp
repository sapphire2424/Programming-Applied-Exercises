/* 2025年度 プログラミング応用演習 第1回 課題プログラム(1) - PAE01_repo1
====================================================================================================
■三目並べ(○×)ゲームのプログラム
・先攻プレーヤーは'O'マーク，後攻プレーヤーは'X'マークとする．
・盤面は3x3とし，横方向をx座標，縦方向をy座標とする．値の範囲は，どちらも{1,2,3]．
  ＊盤面を配列表現する場合，座標(添字)は，{0,1,2}であることに注意
・マークするマスの座標をカンマ区切りで入力する． 例) 1,2
  ＊入力座標値が範囲外の場合や既にマーク済みのマスが選ばれた場合は再入力を促す．
・縦横斜めにマークが三つ並んだ方のプレーヤーが勝ちとする．
・勝者が決まらないまま，全てのマスがマークされたら引き分けとする．
==================================================================================================*/
...
3 |   | O | X
>>> Player-2: ^XE
PS C:\Users\denep\Downloads> cd "c:\Users\denep\Downloads\" ; if ($?) { g++ PAE01_repo1.cpp -o PAE01_repo1 } ; if ($?) { .\PAE01_repo1 }
  | 1 | 2 | 3
--+---+---+---
1 |   |   |
--+---+---+---
